...
%%%%% If you want same velocities everywhere
roads(:,3) = 14;
...
velocityTargetCars = zeros(nbrRuns, length(numberOfTargetCars));
velocityStandardCars = zeros(nbrRuns, length(numberOfTargetCars));
distanceTargetCars = zeros(nbrRuns, length(numberOfTargetCars));
distanceStandardCars = zeros(nbrRuns, length(numberOfTargetCars));
    ...
    for rateTargetCars = 1:length(numberOfTargetCars)
        ...
        while i < numberOfIterations && length(parkedCarIndices) < nbrOfCars
          ...
          if mod(i, 5) == 0
            ...
            saveCurrentVelocity(:,i) = cars(:,currentVelocityIndex);
            saveRoad(:,i) = cars(:,roadIndex);
        ...
        [velocityTargetCars(run,rateTargetCars), velocityStandardCars(run,rateTargetCars)] = getAverageVelocity(saveCurrentVelocity, saveTargetCar, saveRoad);
        [distanceTargetCars(run,rateTargetCars), distanceStandardCars(run,rateTargetCars)] = getAverageDistance(saveRoad, saveTargetCar, roads, nodes);
    end
end
